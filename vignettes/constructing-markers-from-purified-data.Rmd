...
In many situations, marker genes for cell types are either known _a priori_ as expert knowledge, or can be curated through databases such as the [Cellmark](http://biocc.hrbmu.edu.cn/CellMarker/) database. Alternatively, if purified expression data exists (either in bulk or single-cell form), it is possible to quickly derive marker genes using the `findMarkers` function in the [scran](http://bioconductor.org/packages/release/bioc/html/scran.html) R package.
...
We take bulk RNA-seq data from [Holik et al. Nucleic Acids Research 2017](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5389713/) to derive marker genes for 3 different cell lines. This is packaged with `cellassign` as `holik_data`:
...
```{r}
entrez_map <- select(org.Hs.eg.db, as.character(rownames(holik_data$counts)), c("SYMBOL"), "ENTREZID")
...
marker_gene_mat <- expr_mat_thres[(maxdiffs >= quantile(maxdiffs, c(.99))) & (thres_vals <= log(2)),] %>%
  as.matrix
```
